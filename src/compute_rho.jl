...
function compute_initial_condition(mesh_x::AbstractMesh,grid_v::AbstractGrid,k::Float64,T::Float64,u0::Float64)
    ...
    for j in 1:nv
        ...
        for i in 1:(nx + 1)
            x_i = mesh_x.x[i]
            rho[i, j] = f0(x_i, alpha,k,T,u0) / mean_f0(alpha,T,u0)
            u[i, j] = alpha
